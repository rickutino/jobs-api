...
describe('Jobs endpoints', () => {
  ...
  it('should not add a duplicate candidate to a job', async () => {
    ...
    expect(statusCode).toBe(400)
    expect(body.message).toBe('Candidate already added')
  ...
  it('should return 404 when trying to add a candidate to an unexciting job', async () => {
    const candidate = await Candidate.create(candidateFactory.build())
    const unexcitingJobId = jobs[jobs.length - 1].id + 1

    const { body, statusCode } = await supertest(app).post(`/jobs/${unexcitingJobId}/addCandidate`).send({
      candidateId: candidate.id
    })

    expect(statusCode).toBe(404)
    expect(body.message).toEqual('Job vacancy not found')
  });
  ...
  it('should return 404 when trying to remove a candidate from an unexciting job', async () => {
    const candidate = await Candidate.create(candidateFactory.build())
    const unexcitingJobId = jobs[jobs.length - 1].id + 1

    const { body, statusCode } = await supertest(app).post(`/jobs/${unexcitingJobId}/removeCandidate`).send({
      candidateId: candidate.id
    })
